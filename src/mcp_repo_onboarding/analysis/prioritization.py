...
# =============================================================================
# TEMP: Legacy implementations for #77 parity tests.
# Remove these once the refactor is signed off.
# =============================================================================


def _legacy_get_config_priority(path: str) -> int:
    ...
    if name == "makefile" or name == "justfile":
        ...
        score = 200
    elif str(PurePosixPath(path)).startswith(".github/workflows/"):
        score = 150

    if "/" not in str(PurePosixPath(path)):
        score += 100

    return score


def _legacy_get_doc_priority(path: str) -> int:
    p = str(PurePosixPath(path))
    name = PurePosixPath(p).name.lower()
    score = 50
    ...
    if score < 300:
        if p.startswith("docs/") and "/" not in p[5:]:
            score = 250
        elif any(kw in p.lower() for kw in ["quickstart", "install", "setup", "tutorial"]):
            score = 200
        elif p.startswith("docs/"):
            score = 150
    ...
    if any(seg in p.lower() for seg in ["tests/", "test/", "examples/", "scripts/", "src/"]):
        score -= 200

    return score


def _legacy_get_dep_priority(path: str) -> int:
    p = str(PurePosixPath(path))
    name = PurePosixPath(p).name.lower()
    score = 100
    ...
    if any(seg in p.lower() for seg in ["tests/", "test/", "examples/", "scripts/"]):
        score -= 200
