# Repository: /home/rogermt/Paper2Code

## Overview
No Python version pin detected.

## Environment setup
1. Create a virtualenv: `python -m venv .venv` and activate it (`source .venv/bin/activate` on Unix, `.venv\Scripts\activate` on Windows).

## Install dependencies
1. Install dependencies with pip: `pip install -r requirements.txt`.

## Run / develop locally
- `bash scripts/run.sh`
- `bash scripts/run_latex.sh`
- `bash scripts/run_latex_llm.sh`
- `bash scripts/run_litellm.sh`
- `bash scripts/run_llm.sh`

## Run tests
No explicit commands detected by the analyzer.

## Lint / format
No explicit commands detected by the analyzer.

## Dependency files detected
- `requirements.txt`

## Useful configuration files
- `.pre-commit-config.yaml`

## Useful docs
- `CONTRIBUTING.md`
- `README.md`
- `data/paper2code/README.md`
- `notebooks/README.md`


Token usage: unknown (33,858)